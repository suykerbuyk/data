...
set_mach2_props() {
	...
	for DEV in $(find $DSK_PATH -name "${MACH2_DSK_PREFIX}*" | grep -v part | cut -c 1-38 | sort -u ); do
		...
		set_drv_wce "/dev/${KDEV0}"
		set_drv_wce "/dev/${KDEV1}"
		set_drv_queue "${KDEV0}"
		set_drv_queue "${KDEV1}"
...
print_drv_inventory
do_sanity_check
[[ ${EVANS_DRV_COUNT} > 0 ]] && set_evans_props
[[ ${MACH2_DRV_COUNT} > 0 ]] && set_mach2_props
